        ...
        for si, pred in enumerate(preds):
            ...
            if npr == 0:
                if nl:
                    ...
                    if self.args.plots:
                        self.confusion_matrix.process_batch(detections=None, gt_bboxes=bbox, gt_cls=cls, im_file=batch["im_file"][si])  #检测不到目标
                continue  #下一个样本
            ...
            if self.args.plots:
                self.confusion_matrix.process_batch(predn, bbox, cls, im_file=batch["im_file"][si])  #检测到目标
