...
def bbox2dist(anchor_points, bbox, reg_max):
    ...
    Args:
        ...
        bbox(Tensor)：(b,n_anchors, 4) xyxy  每个描点对应的真实框
        reg_max（int）：
    Returns:
...
def dist2rbox(pred_dist, pred_angle, anchor_points,dim=-1):
    ...
    x, y = xf * cos - yf * sin, xf *sin + yf * cos   #中心点到描点的xy方向距离
...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        Args:
            ...
            anc_points(Tensor): shape(n_anchors, 2)    描点中心点 -> 描点
        ...
        Returns:
            ...
            fg_mask(Tensor): shape(bs, n_anchors)      描点是否有对应的真实框
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        #获取位于真实框内且IoU指标前topk个的描点mask   每个描点都会对应一个预测框，根据预测款计算Iou
        #(b, n_mboxes, n_anchors) (b, n_mboxes, n_anchors)
        mask_pos, align_metric, overlaps = self.get_pos_mask(pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt)  #获取在真实框内的前topk个描点 - 1个真实框对应topk个描点
        #(b, n_anchors) (b, n_anchors) (b, n_mboxes, n_anchors)
        target_gt_idx, fg_mask, mask_pos = self.select_highest_overlaps(mask_pos, overlaps, self.n_max_boxes)   #获取描点和真实框1对1的mask
        #(b, n_anchors) (b, n_anchors, 4) (b, n_anchors, num_classes)
        target_labels, target_bboxes, target_scores = self.get_targets(gt_labels, gt_bboxes, target_gt_idx, fg_mask)   #获取各个描点对应的真实框种类、位置、one_hot
    ...
    def get_pos_mask(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, anc_points, mask_gt):
        """获取每个真实框其内的描点，且根据种类分数和iou指标只取前topk个描点"""
        mask_in_gts = self.select_candidates_in_gts(anc_points, gt_bboxes)  #获取在真实框内的描点mask  (bs, n_mboxes, n_anchors)   一个真实框对应n_anchors个描点
        #(b, n_mboxes, n_anchor) = (b,n_mboxes, n_anchors)*(b, n_mboxes,1)
        mask  = mask_in_gts * mask_gt       #乘以真实框的掩膜获取各真实框对应的描点 -- 一对n
        #(b, n_mboxes, n_anchors)，(b, n_mboxes, n_anchors)
        align_metric, overlaps = self.get_box_metrics(pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask)   #获取预测框和真实框的Iou指标   每一个真实框对应n个描点
        #(b, n_mboxes, n_anchors)
        mask_topk = self.select_topk_candidates(align_metric, topk_mask=mask_gt.expand(-1, -1, self.topk).bool())   #每个真实框取前topk个最大指标描点的mask  一对topk
    ...
    def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
        """
        获取位于真实框内的描点的mask，因为一个描点可能同时在多个真实框内，所以每个真实框跟n个描点计算是否在框内，结果（b, n_mboxes, n_anchors）
        Args：
            xy_centers(Tensor): shape(n_anchors, 2)描点
        ...
        bbox_deltas = torch.cat((xy_centers[None] - lt, rb - xy_centers[None]), dim=2)    #shape(n_mboxes*bs, n_anchors, 4)  在真实框内的描点 delta大于0
        bbox_deltas = bbox_deltas.view(bs, n_boxes, n_anchors, -1)   #b, n_mboxes, n_anchors, 4
        return bbox_deltas.amin(3).gt_(eps)   #获取deltas的最小值并与eps比较，小于eps的为0，排除不在真实框内的描点（b, n_mboxes, n_acnhors）
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        for k in range(self.topk):
            count_tensor.scatter_add_(-1, topk_idxs[:, : , k:k+1], ones)   #每个真实框对应指定索引的描点数量+1+1+1
        count_tensor.masked_fill_(count_tensor > 1, 0)   #去除真实框外的(0)和在候选外的描点(1)均替换为0，其余的都是根据topk指标选上的描点,每个真实框对应topk个描点
        return count_tensor.to(metrics.dtype)   #(b, n_mboxes, n_anchors) 每个真实框最多对应topk个描点，即n_anchors个数据中最多只有topk个不为0

    @staticmethod
    def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
        """
        如果一个描点对应多个真实框，有着最高IOU的真实框将被选中
        Args：
            mask_pos(Tensor): shape(b, n_mboxes,n_anchors)
            overlaps(Tensor): shape(b, n_mboxes, n_anchors)
        Return:
            target_gt_idx(Tensor): shape(b, n_anchors)  描点对应真实框的索引
            fg_mask(Tensor): shape(b, n_anchors)      每个描点对应的真实框数量
            mask_pos(Tensor): shape(b, n_mboxes, n_anchors)  一个描点对应一个真实框的mask
        """
        fg_mask = mask_pos.sum(-2)  #（b,n_anchors） 描点对应真实框数量
        if fg_mask.max() > 1:  #一个描点匹配了多个真实框
            mask_multi_gts = (fg_mask.unsqueeze(1) > 1).expand(-1, n_max_boxes, -1)   #（b, n_mboxes， n_anchors）  匹配多个真实框的描点为1
            ...
            is_max_overlaps.scatter_(1, max_overlaps_idx.unsqueeze(1), 1)   #（b, n_mboxes, n_anchors）  将最大iou的描点置为1 ，其余为0

            mask_pos = torch.where(mask_multi_gts, is_max_overlaps, mask_pos).float()    #(b, n_max_boxes, n_anchors) 匹配多个真实框的描点取is_max_overlaps对应位置的值，其余的取mask_pos原值
            fg_mask = mask_pos.sum(-2)  #(b, n_anchors)   描点有框的为1，无框的为0
        target_gt_idx = mask_pos.argmax(-2)  #(b, n_anchors)  每个描点对应的真实框索引
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        ...
        Args:
            ...
            target_gt_idx(Tensor): 描点对应真实框的索引（b,n_anchors)
            fg_mask（Tensor）：一个布尔值张量，显示有描点的位置（b, n_anchors）
        Returns:
            target_labels(Tensor): 对应正描点的真实框种类 （b, n_anchors）
            target_bboxes(Tensor): 对应正描点的真实框位置 （b, n_anchors, 4)
        ...
        target_labels = gt_labels.long().flatten()[target_gt_idx]     #获取描点对应真实框种类标签 (b, n_anchors)

        target_bboxes = gt_bboxes.view(-1, gt_bboxes.shape[-1])[target_gt_idx]  #获取描点对应真实框位置标签（b, n_anchors, 4)
        ...
        target_scores = torch.where(fg_scores_mask > 0, target_scores, 0)   #描点没有对应真实框的全归为0
